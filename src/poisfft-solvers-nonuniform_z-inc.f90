    subroutine PoisFFT_Solver3D_nonuniform_Z_PPNs(D, Phi, RHS)
      ...
      do k = 1, D%nz
          ...
          call Execute_MPI(D%Solvers2D(tid)%forward, D%Solvers2D(tid)%cwork)

          D%cwork(:,:,k) = D%Solvers2D(tid)%cwork
      end do

      if (D%Solvers1D(3)%mpi_transpose_needed) then
        call solve_tridiagonal_1d_complex_z_cwork(D%Solvers1D(3), D)
      else
        !$omp do private(lam)
        do j = 1, D%ny
          do i = 1, D%nx
      ...
      do k = 1, D%nz
          ...
          call Execute_MPI(D%Solvers2D(tid)%backward, D%Solvers2D(tid)%cwork)
...
#undef m       
  ...
  ! solve for x from the vectors c-prime and x-prime
    ...
    subroutine solve_tridiagonal_1d_complex_z_cwork(D1D, D)
      type(PoisFFT_Solver1D), intent(inout), target :: D1D
      type(PoisFFT_Solver3D_nonuniform_Z), intent(inout), target :: D
      complex(CP), allocatable :: cwork2(:,:,:), ctmp1(:,:,:), ctmp2(:)
      integer :: nx, ny, nz
      integer :: i,j,k,l, ie
      integer :: glob_i
      integer :: tid
...
#undef m
  ...
  ! solve for x from the vectors c-prime and x-prime
    ...
    end subroutine solve_tridiagonal_1d_complex_z_cwork
